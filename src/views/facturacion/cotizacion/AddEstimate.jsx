...
import ModalCreateCliente from '../../../components/modals/ModalCreateTercero';
...
const AddEstimate = () => {
    ...
    const [dataPrefijo, setDataPrefijo] = useState('COT');
    ...
    const handleGuardarFacturaCabecera = () => {
        ...
        formDataCabeceraFactura.append('order_total_before_tax', subtotal);
        formDataCabeceraFactura.append('descuento', descuento);
        ...
        const token = sessionStorage.getItem('token');
        fetch('https://api.invoix.co/v1/saveprices', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`
            },
    ...
    const handleSaveDetails = () => {
        ...
        const token = sessionStorage.getItem('token');
        fetch('https://api.invoix.co/v1/savedetailprices', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            ...
            .then(response => {
                if (response.ok) {
                    // Estado 200 - éxito
                    Swal.fire('Éxito', 'Cotizacion Guardada con éxito', 'success');
                    navigate('/estimate'); // Navegar a la ruta "/invoice"
                } else if (response.status === 400) {
                    // Estado 400 - error de solicitud
                    Swal.fire('Error', 'Error en la solicitud', 'error');
                } else if (response.status === 500) {
                    // Estado 500 - error interno del servidor
                    Swal.fire('Error', 'Error interno del servidor solo se envio cabecera ', 'error');
                } else {
                    // Otros errores
                    Swal.fire('Error', 'Error desconocido', 'error');
                }
    ...
    return (
        <main className="">
            <div className="px-16 mt-5 flex justify-start">
                <h2 className="font-semibold text-gray-600 text-4xl">Crear Cotizacion</h2>
            </div>
            <div class="pt-6 px-4">
                        ...
                        <div className='w-[1500px] h-full  bg-gray-50 shadow-2xl rounded-lg z-10'>
                            ...
                            <div className='w-full flex justify-around px-20 pt-20'>
                                ...
                                <div className='flex items-center justify-end w-4/12 '>
                                    ...
                                    <input className='w-2/6 h-10 px-5 rounded-md bg-gray-100' disabled type="text" value={"Creando..."} />
                                </div>
                            </div>
                ...
                <ModalCreateCliente showCrearCliente={showCrearCliente} setShowCrearCliente={setShowCrearCliente} />
            </div>
